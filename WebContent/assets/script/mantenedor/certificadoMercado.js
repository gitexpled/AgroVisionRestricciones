var TableDatatablesAjax = function() {
	...
	var handleDemo1 = function() {
		...
		grid.init({
					...
					dataTable : { 
						...
						"ajax" : {
							"url" : "/HortiSafeOlmos/"+"json/certificadoMercado/view", // ajax
																				// source
						},
						"columnDefs" : [
								{
	...
	var setEstado = function(){
		$("body").on("click",".changeStatus",function(e){
			...
			$.ajax({
				type : 'GET',
				url : "/HortiSafeOlmos/"+"json/certificado/setStatus",
				data : {
					id: id
				},
	...
	var obtener = function() {
		...
		$("#modal-modifica-cuenta").on(
				...
				function(e) {
					...
					$.ajax({
						type : 'GET',
						url : "/HortiSafeOlmos/json/certificadoMercado/" + id,
						data : "",
						success : function(data) {
							console.log(data);
							$("#updateNombre").val(data.idcertificado);
	...
	var editar = function() {
		...
		form1.validate({
					...
					submitHandler : function(form) {
						...
						row.id = ID;
						row.idcertificado = $("#updateNombre").val();
						row.idmercado = $("#updateMercado").val();

						$.ajax({
									url : "/HortiSafeOlmos/"+"json/certificadoMercado/put",
									type : "PUT",
		...
		form1.validate({
					...
					rules : {
						...
						regUser: {
							...
							rangelength : [4, 50],
							remote: {
								url: "/HortiSafeOlmos/"+"json/certificado/validaUserName",
								type: "POST",
								data: {
									user: function(){
										return $('#regUser').val();
					...
					submitHandler : function(form) {
						...
						row.idcertificado = $('#updateNombre2').val();
						row.idmercado = $('#updateMercado2').val();
						console.log(row);

						$.ajax({
									url : "/HortiSafeOlmos/"+"json/certificadoMercado/insertCertificado",
									type : "PUT",
									data : JSON.stringify(row),
									beforeSend : function(xhr) {
										xhr.setRequestHeader("Accept",
...
jQuery(document).ready(function() {
	...
	$.ajax({
		url : "/HortiSafeOlmos/"+"json/certificado/getCertificados",
		type : "GET",
		data : "",
		beforeSend : function(xhr) {
			xhr.setRequestHeader("Accept",
	...
	$.ajax({
		url : "/HortiSafeOlmos/"+"json/mercado/getAllOutFilter",
		type : "GET",
		data : "",
		beforeSend : function(xhr) {
			xhr.setRequestHeader("Accept",
...
function deleteM(id){
	var t = {
		TABLE: "certificadoMercado",
		WHERE: {
			id: id
		}
	}
	console.log(t)
	Delete(t).then(function(res){
		if(res.error == 0){
			alert("Eliminado");
			location.reload();
		}else{
