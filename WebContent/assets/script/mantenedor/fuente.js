	...
	var handleDemo1 = function() {
		...
		grid.init({
					...
					dataTable : { 
						...
						"ajax" : {
							"url" : "/AgroVisionRestricciones/"+"json/fuente/view", // ajax
																				// source
						},
						"columnDefs" : [
								{
	...
	var obtener = function() {
		$("#modal-modifica-fuente").on(
				...
				function(e) {
					...
					$.ajax({
						type : 'GET',
						url : "/AgroVisionRestricciones/json/fuente/" + id,
						data : "",
						success : function(data) {
							console.log(data)
							$("#updateFuente").val(data.nombre);
	...
	var editar = function() {
		...
		form1.validate({
					...
					submitHandler : function(form) {
						...
						row.nombre = $('#updateFuente').val();
						row.idFuente = ID;
						$.ajax({
									url : "/AgroVisionRestricciones/"+"json/fuente/put",
									type : "PUT",
	...
	var insertFuente = function(){
		...
		form1.validate({
					...
					submitHandler : function(form) {
						...
						row.nombre = $('#regFuente').val();
						console.log(row);

						$.ajax({
									url : "/AgroVisionRestricciones/"+"json/fuente/insertFuente",
									type : "PUT",
									data : JSON.stringify(row),
									beforeSend : function(xhr) {
										xhr.setRequestHeader("Accept",
									...
									success : function(data, textStatus, jqXHR) {
										...
										$('#regFuente').val("");
									swal({
										  title: 'Fuente ingresada exitosamente!',
										  animation: true,
										  customClass: 'animated tada'
										})
										var table = $('#datatable_ajax')
