	...
	var handleDemo1 = function() {
		...
		grid.init({
					...
					dataTable : { 
						...
						"ajax" : {
							"url" : "/AgroVisionRestricciones/"+"json/diccionario/view", // ajax
																				// source
						},
						"columnDefs" : [
								{
	...
	var obtener = function() {
		$("#modal-modifica-diccionario").on(
				...
				function(e) {
					...
					$.ajax({
						type : 'GET',
						url : "/AgroVisionRestricciones/json/diccionario/" + id,
						data : "",
		...
		form1.validate({
					...
					rules : {
						...
						updatecodRem : {
							...
							alfanumerico : true,
							remote: {
								url: "/AgroVisionRestricciones/"+"json/diccionario/validaDiccionario",
								type: "POST",
								data: {
					...
					submitHandler : function(form) {
						...
						row.codReemplazo = $("#updatecodRem").val();
						row.idDiccionario = ID;

						$.ajax({
									url : "/AgroVisionRestricciones/"+"json/diccionario/put",
									type : "PUT",
	...
	var insertDiccionario = function(){
		...
		form1.validate({
					...
					submitHandler : function(form) {
						...
						row.codReemplazo = $('#regCodigoRem').val();
...
jQuery(document).ready(function() {
	$.getJSON("/AgroVisionRestricciones/json/diccionario/getAllOutFilterEqual",function(data){
		cadena = "<option value=''>Seleccionar</option>";
		$(data).each(function(key,val){
			cadena += "<option value='"+val.codProducto+"'>"+val.codProducto+"</option>";
		})
		$('#regCodigoProd').html(cadena);
	})
