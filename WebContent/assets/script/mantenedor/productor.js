var TableDatatablesAjax = function() {
	...
	var obtener = function() {
		$("#modal-modifica-productor").on(
				...
				function(e) {
					...
					$.ajax({
						type : 'GET',
						url : "/AgroVisionRestricciones/json/productor/" + id,
						data : "",
						success : function(data) {
		...
		form1.validate({
					...
					messages : {
						...
						updateNombre : {
							...
							rangelength : "Debe ser mayor a 3 y menor a 50",
							alfanumerico : "Ingrese sólo valores alfanumericos"
						},
						updateCodSap : {
							required : "Este campo es obligatorio",
							rangelength : "Debe ser mayor a 4 y menor a 4",
							alfanumerico : "Ingrese sólo valores alfanumericos"
						}
					...
					submitHandler : function(form) {
						...
						row.codSap = $("#updateCodSap").val();
						
						$.ajax({
									url : "/AgroVisionRestricciones/"+"json/productor/put",
									type : "PUT",
	...
	var insertProductor = function(){
		...
		form1.validate({
					...
					messages : {
						...
						codSap : {
							required: "Este campo es obligatorio",
							rangelength : "No debe ser menor a 4 y mayor a 4 caracteres",
							alfanumerico : "ingrese valores alfanumericos"
						}
					...
					submitHandler : function(form) {
						...
						row.idUser = $("#idUserPefil").val();
						row.codSap = $('#codSap').val();
						

						$.ajax({
									url : "/AgroVisionRestricciones/"+"json/productor/insertProductor",
									type : "PUT",
									data : JSON.stringify(row),
									beforeSend : function(xhr) {
										xhr.setRequestHeader("Accept",
	...
	var drop = function(){
		$("body").on("click",".drop",function(e){
			...
			$.ajax({
				type : 'GET',
				url : "/AgroVisionRestricciones/"+"json/productor/drop",
				data : {
					id: id
				},
				success : function(data) {
