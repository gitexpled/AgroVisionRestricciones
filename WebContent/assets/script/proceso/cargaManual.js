...
function agregarFila() {
	...
	$("#tblPeticion")
		.append(
			...
			+ "<td class='tdlimite'  style='text-align: center;'>" + limiteInpu.replace(",", ".") + "</td>"
...
function Enviar() {
	...
	if (nFilas == 0) {
		...
		for (var i = 0; i < nFilas; i++) {
			...
			data.limite = Array_limiteInput[i].replace(",", ".");
	...
	$.ajax({
		url: "/AgroVisionRestricciones/" + "json/cargaManual/add",
		type: "PUT",
		data: search,
...
function actualizar() {
	...
	$.ajax({
		url: "/AgroVisionRestricciones/" + "json/cargaManual/updateAnalisis",
		type: "PUT",
	...
	var getView = function () {
		...
		grid.init({
			...
			dataTable: {
				...
				"ajax": { "url": "/AgroVisionRestricciones/" + "json/cargaManual/view" },
				"columnDefs": [
	...
	var getId = function () {
		$("#modal-update").on(
			...
			function (e) {
				...
				$.ajax({
					type: 'GET',
					url: "/AgroVisionRestricciones/" + "json/cargaManual/" + id,
					data: "",
					success: function (data) {
						...
						$.ajax({
							...
							success: function (res) {
								...
								$(res.data).each(function (key, val) {
									let selected = val.codigo == codParcela? "selected": ""
						...
						$.ajax({
							...
							success: function (res) {
								...
								$(res.data).each(function (key, val) {
									let selected = val.codTurno == codTurno? "selected": ""
