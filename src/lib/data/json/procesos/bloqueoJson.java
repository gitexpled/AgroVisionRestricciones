...
public class bloqueoJson {
	...
	public @ResponseBody dataTable getView(HttpServletRequest request,HttpSession httpSession)  {
		...
		try {
			datas = bloqueadoDB.getAll(filter, "", start, length);

			Iterator<bloqueo> f = datas.iterator();

			data.setRecordsFiltered(bloqueadoDB.getAllcount(filter));
			data.setRecordsTotal(bloqueadoDB.getAllcount(filter));

			while (f.hasNext()) {
				bloqueo row = f.next();
				String[] d = { row.getIdBloqueo()+"",row.getCodProductor(),row.getMercado(),row.getEspecie(),row.getB(),row.getActivo() };
	...
	@RequestMapping(value = "/bloqueo/add" , method= {RequestMethod.PUT}, produces = MediaType.APPLICATION_JSON_VALUE)
	...
	public @ResponseBody bloqueo getId(@PathVariable String codigo,HttpSession httpSession) throws Exception {
		...
		bloqueo row = bloqueadoDB.getId(codigo);

		return row;

	}
	
	@RequestMapping(value = "/bloqueo/put", method = RequestMethod.PUT, produces = MediaType.APPLICATION_JSON_VALUE)
	...
	@RequestMapping(value = "/bloqueo/getAllOutFilter", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
