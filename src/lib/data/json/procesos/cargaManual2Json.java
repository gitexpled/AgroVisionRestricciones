...
@Controller
public class cargaManual2Json {

	@RequestMapping(value = "/cargaManual2/view", method = { RequestMethod.POST, RequestMethod.GET })
	public @ResponseBody dataTable getView(HttpServletRequest request,HttpSession httpSession)  {
		...
		System.out.println("GET 2222222222:::::::::::::::::::::::::::::::::::::::: ");
		...
		try {
			datas = cargaManual2DB.getAll(filter, "", start, length);

			Iterator<cargaManual> f = datas.iterator();

			data.setRecordsFiltered(cargaManual2DB.getAllcount(filter));
			data.setRecordsTotal(cargaManual2DB.getAllcount(filter));

			while (f.hasNext()) {
				cargaManual row = f.next();
				
				String[] d = { row.getIdCargaManual()+"",row.getCodProductor(),row.getFecha(),row.getLaboratorio(),row.getIdentificador() };
	...
	@RequestMapping(value = "/cargaManual2/add" , method= {RequestMethod.PUT}, produces = MediaType.APPLICATION_JSON_VALUE)
	public @ResponseBody boolean add(@RequestBody cargaManual row,HttpSession httpSession) throws ParseException
	{
		...
		if (ses.isValid()) {
			...
			resp = cargaManual2DB.insert(row);
	...
	@RequestMapping(value = "/cargaManual2/{codigo}", method = { RequestMethod.GET })
	public @ResponseBody cargaManual getId(@PathVariable String codigo,HttpSession httpSession) throws Exception {
		...
		cargaManual row = cargaManual2DB.getId(codigo);

		return row;

	}
	
	@RequestMapping(value = "/cargaManual2/put", method = RequestMethod.PUT, produces = MediaType.APPLICATION_JSON_VALUE)
	...
	@RequestMapping(value = "/cargaManual2/getAllOutFilter", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	...
	@RequestMapping(value = "/cargaManual2/updateAnalisis", method = RequestMethod.PUT, produces = MediaType.APPLICATION_JSON_VALUE)
	public @ResponseBody boolean UpdateAnalisis(@RequestBody  cargaManual row,HttpSession httpSession) throws Exception {
		...
		return cargaManual2DB.UpdateAnalisis(row);
